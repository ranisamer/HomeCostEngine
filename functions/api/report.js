...
function calculate(type, raw={}){
  ...
  if(type==="rooftearoff"){
    const area=finite(raw.rtoArea,1,1e8),layers=Math.round(finite(raw.rtoLayers,1,3)),material=String(raw.rtoMaterial||"asphalt"),dump=finite(raw.rtoDump,0,1e7);
    const rates={asphalt:[1,3],metal:[1,3],wood:[1.5,3.5],tile:[2,5],slate:[2,5]}; if(!Number.isFinite(area)||!rates[material]) throw new Error("Check your tear-off inputs before sending the report.");
    const factor=1+(layers-1)*.55,rl=rates[material][0],rh=rates[material][1],low=area*rl*factor+dump,high=area*rh*factor+dump;
    return {title:"Roof Tear-Off Cost Report",subject:"Your roof tear-off plan: "+money(low)+"–"+money(high),url:"https://homecostengine.com/calculators/roof-tear-off-cost.html",summary:"Your entered roof area and layer count produce a tear-off planning range of "+money(low)+" to "+money(high)+".",metrics:[["Roof area",number(area,0)+" sq ft"],["Roofing squares",number(area/100,1)],["Existing layers",String(layers)],["Existing material",material],["Disposal allowance",money(dump)],["Planning range",money(low)+" – "+money(high)]],note:"Removal cost varies with material, layer count, pitch, stories, access, staging and local disposal fees. Replace the planning assumptions with written local pricing before signing a contract."};
  }
  if(type==="roofunderlayment"){
    const area=finite(raw.ruArea,1,1e8),waste=finite(raw.ruWaste,1,2),coverage=finite(raw.ruCoverage,1,1e6),rollPrice=finite(raw.ruRollPrice,0,1e6),laborRate=finite(raw.ruLaborRate,0,1e5);
    if(![area,waste,coverage,rollPrice,laborRate].every(Number.isFinite)) throw new Error("Check your underlayment inputs before sending the report.");
    ...
    return {title:"Roof Underlayment Cost Report",subject:"Your underlayment plan: "+money(total),url:"https://homecostengine.com/calculators/roof-underlayment-cost.html",summary:"Your entered roll coverage, material price and labor rate produce a planning total of "+money(total)+".",metrics:[["Roof area",number(area,0)+" sq ft"],["Overlap / waste",number((waste-1)*100,0)+"%"],["Order area",number(orderArea,0)+" sq ft"],["Estimated rolls",String(rolls)],["Material cost",money(material)],["Labor allowance",money(labor)],["Planning total",money(total)],["2026 broad installed benchmark",money(area*.60)+" – "+money(area*2.70)]],note:"The benchmark uses a broad published national installed range. Product type, roof covering, slope, climate, overlap, leak-barrier zones and manufacturer requirements can change the final scope."};
  }
  if(type==="roofflashing"){
    const lf=finite(raw.rfLength,0,1e7),rate=finite(raw.rfRate,0,1e5),chimneys=finite(raw.rfChimneys,0,1000),chimneyCost=finite(raw.rfChimneyCost,0,1e6),vents=finite(raw.rfVents,0,10000),ventCost=finite(raw.rfVentCost,0,1e6),skylights=finite(raw.rfSkylights,0,1000),skylightCost=finite(raw.rfSkylightCost,0,1e6);
    if(![lf,rate,chimneys,chimneyCost,vents,ventCost,skylights,skylightCost].every(Number.isFinite)) throw new Error("Check your flashing inputs before sending the report.");
    ...
    return {title:"Roof Flashing Cost Report",subject:"Your roof flashing plan: "+money(total),url:"https://homecostengine.com/calculators/roof-flashing-cost.html",summary:"Your entered flashing quantities and unit rates produce a planning total of "+money(total)+".",metrics:[["Linear flashing",number(lf,0)+" ft × "+money(rate)],["Linear flashing allowance",money(linear)],["Chimney flashing",money(chimney)],["Vent boots / penetrations",money(vent)],["Skylight flashing",money(sky)],["Planning total",money(total)]],note:"Flashing is not one uniform line item. Valleys, walls, chimneys, skylights and penetrations use different details and labor. Verify what will be replaced, reused or fabricated in the written roofing scope."};
  }
  if(type==="roofdripedge"){
    const eave=finite(raw.rdeEave,0,1e7),rake=finite(raw.rdeRake,0,1e7),waste=finite(raw.rdeWaste,1,2),pieceLength=finite(raw.rdePieceLength,1,100),rate=finite(raw.rdeRate,0,1e5);
    if(![eave,rake,waste,pieceLength,rate].every(Number.isFinite) || eave+rake<=0) throw new Error("Check your drip-edge inputs before sending the report.");
    const base=eave+rake,orderFeet=base*waste,pieces=Math.ceil(orderFeet/pieceLength),billableFeet=pieces*pieceLength,total=orderFeet*rate,low=orderFeet*1.5,high=orderFeet*9;
    return {title:"Roof Drip Edge Cost Report",subject:"Your drip edge plan: "+money(total),url:"https://homecostengine.com/calculators/roof-drip-edge-cost.html",summary:"Your entered eave and rake lengths produce about "+number(orderFeet,0)+" linear feet of drip edge including allowance.",metrics:[["Eave length",number(eave,0)+" ft"],["Rake length",number(rake,0)+" ft"],["Allowance",number((waste-1)*100,0)+"%"],["Planned linear feet",number(orderFeet,0)+" ft"],["Pieces to purchase",String(pieces)+" × "+number(pieceLength,0)+" ft"],["Purchased length",number(billableFeet,0)+" ft"],["Entered installed rate",money(rate)+" / linear ft"],["Planning total",money(total)],["Broad 2026 replacement benchmark",money(low)+" – "+money(high)]],note:"The broad benchmark uses a published drip-edge replacement range. Fascia condition, removal, metal type, corners, roof height, access and whether the work is part of a full reroof can change the installed rate."};
  }
  if(type==="roofventilation"){
    const ridgeLf=finite(raw.rvRidgeLf,0,1e7),ridgeRate=finite(raw.rvRidgeRate,0,1e5),boxCount=finite(raw.rvBoxCount,0,1e4),boxCost=finite(raw.rvBoxCost,0,1e6),soffitLf=finite(raw.rvSoffitLf,0,1e7),soffitRate=finite(raw.rvSoffitRate,0,1e5),fanCount=finite(raw.rvFanCount,0,1e4),fanCost=finite(raw.rvFanCost,0,1e6);
    if(![ridgeLf,ridgeRate,boxCount,boxCost,soffitLf,soffitRate,fanCount,fanCost].every(Number.isFinite)) throw new Error("Check your roof ventilation inputs before sending the report.");
    const ridge=ridgeLf*ridgeRate,box=boxCount*boxCost,soffit=soffitLf*soffitRate,fans=fanCount*fanCost,total=ridge+box+soffit+fans;
    return {title:"Roof Ventilation Cost Report",subject:"Your roof ventilation plan: "+money(total),url:"https://homecostengine.com/calculators/roof-ventilation-cost.html",summary:"Your entered ventilation quantities and unit rates produce a planning total of "+money(total)+".",metrics:[["Ridge vent",number(ridgeLf,0)+" ft • "+money(ridge)],["Soffit vent",number(soffitLf,0)+" ft • "+money(soffit)],["Box / static vents",number(boxCount,0)+" • "+money(box)],["Powered vents",number(fanCount,0)+" • "+money(fans)],["Planning total",money(total)]],note:"This is a cost builder, not a ventilation sizing calculation. Ask a qualified roofing or building professional to verify required intake and exhaust area, product net-free vent area, roof assembly and local requirements before selecting quantities."};
  }
  if(type==="concrete"){
